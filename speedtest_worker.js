    ...
    if (params[0] === 'start' && testState === -1) {
        ...
        try {
            ...
            try {
                ...
                twarn(
                    'Error parsing custom settings JSON. Please check your syntax'
                );
            }
            ...
            twarn(
                'Possible error in custom test settings. Some settings might not have been applied. Exception: ' +
                    e
            );
        }
...
function dlTest(done) {
    ...
    interval = setInterval(
        function () {
            tverb(
                'DL: ' + dlStatus + (graceTimeDone ? '' : ' (in grace time)')
            );
        ...
        interval = setInterval(
            function () {
                ...
                if (!graceTimeDone) {
                    ...
                    if (
                        (t + bonusT) / 1000.0 > settings.time_ul_max ||
                        failed
                    ) {
